...
class HMM:
    def __init__(self, train_data, test_data):
        ...
        self.viterbi = {} # changed to dictionary
        self.backpointer = {} # changed to dictionary
    ...
    def elprob(self,state,word):
        ...
        return math.log2(self.emission_PD[state].prob(word)) # fixme
    ...
    def initialise(self, observation):
        ...
        for state in self.states:
            viterbi_value = ((-self.tlprob('<s>', state))+(-self.elprob(state, observation))) # viterbi value of each state's transition from <s> and observation given each state
            self.viterbi[state] = [viterbi_value] # Initialise the viterbi dictionary which looks like {state1:[v1, v2, ...], state2:[v1, v2, ...], ...., staten:[v1, v2, ...]}
            self.backpointer[state] = ['<s>'] # all the backpointer for each state at time t = 0 points to <s> as <s> is the start of each sentence
    ...
    def tag(self, observations):
        ...
        for t in range(1, len(observations)): # fixme to iterate over steps
            for s in self.states: # fixme to iterate over states
                ...
                min_viterbi_value = -1
                min_backpointer_state = '' # keeps track of the previous state with min viterbi cost
                for sprev in self.states: # go through the previous tags in order to find the min viterbi cost
                    temp_viterbi_value = ((self.viterbi[sprev][t-1])+(-self.tlprob(sprev, s))+(-self.elprob(s, observations[t])))
                    if min_viterbi_value == -1 or min_viterbi_value > temp_viterbi_value: # update the minimun
                        #update both min values
                        min_viterbi_value = temp_viterbi_value
                        min_backpointer_state = sprev
                self.viterbi[s].append(min_viterbi_value) # append min viterbi cost to our viterbi dictionary
                self.backpointer[s].append(min_backpointer_state) # append the backpointer/ previous state to our backpointer dictionary
        ...
        min_viterbi_value = -1
        min_backpointer_state = '' # keeps track of the previous state with min viterbi cost
        for sprev in self.viterbi:
            temp_viterbi_value = ((self.viterbi[sprev][-1])+(-self.tlprob(sprev, '</s>'))) # only the transition probability to the final state </s> is taken into consideration
            if min_viterbi_value == -1 or min_viterbi_value > temp_viterbi_value: # update the minimum
                        #update both min values
                        min_viterbi_value = temp_viterbi_value
                        min_backpointer_state = sprev
        ...
        tags = [] # fixme
        tag = min_backpointer_state # current backpointer
        for t in reversed(range(len(observations))):
            tags.insert(0, tag) # append the current backpointer to the front of the list
            tag = self.get_backpointer_value(tag, t) # back track using the backpointer dictionary
        return tags
    ...
    def get_viterbi_value(self, state, step):
        ...
        return self.viterbi[state][step] # fix me
    ...
    def get_backpointer_value(self, state, step):
        ...
        return self.backpointer[state][step] # fix me
...
def answer_question4b():
    ...
    answer =  inspect.cleandoc("""Even though, the emission probability would suggest that gaudy should be ADJ, but the transition probability of VERB to ADV is much higher than that of VERB to ADJ in the model, thus making the biased POS tagger(after training on the corpus data) think gaudy is ADV.""")[0:280]
...
def answer_question5():
    ...
    return inspect.cleandoc("""The parser has 100% construction coverage, thus minimal attachment ambiguity. The POS tagger can be used to get the tags for each word, even the ones that have not been seen before. The tagger gives the best estimation of the tag, reducing POS ambiguity. Thus, tagger helps parser overcome the problem of unseen word tags and helps it to provide a parse for any well-formed sentence. Alone, the parser would do worse due to POS ambiguity, except for when tagger gets a wrong series of tags.""")[0:500]
...
def answer_question6():
    ...
    return inspect.cleandoc("""Total tags in Universal tagset is much less compared to Brown. Thus, there is a higher probability of seeing most of the constructions even with our small training corpus. This helps improve the accuracy of our model. If Brown tagset was used then we would encounter data sparsity by not seeing enough constructions to make good estimations using our model. Some tags might not be seen at all while training, making our model highly inaccurate. Larger training corpus would solve this problem.""")[0:500]
...
def answers():
    ...
    train_data_universal = tagged_sentences_universal[0:train_size] # fixme
